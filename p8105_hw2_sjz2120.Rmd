...
The resulting merged dataset `merged_df` contains a total of **`r nrow(merged_df)` rows/observations** and **`r ncol(merged_df)` columns/variables**. Key in the cleaned/tidied `merged_df` dataset include: **`r colnames(merged_df)`**; a description of each variable is listed below:
